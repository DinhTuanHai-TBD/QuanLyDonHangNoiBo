...
Để test nhanh API, em chuẩn bị file:
...
3. Gọi request đăng nhập.
4. Copy `accessToken`.
5. Gắn token vào header `Authorization`.
6. Test các nhóm API còn lại.
...
Nếu dùng PowerShell, có thể test bằng:
...
Sau khi đăng nhập bằng `admin@demo.vn`, có thể test theo thứ tự:
...
- Nên import Postman collection để test nhanh.
